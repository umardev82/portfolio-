    ...
    <section id="contact" className="py-24 px-4 sm:px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid lg:grid-cols-2 gap-10">
          ...
          >
            {/* Availability badge */}
            <div className="glass-card p-5 flex items-center gap-4 border border-emerald-500/25 bg-emerald-500/[0.04]">
              <div className="w-10 h-10 rounded-xl bg-emerald-500/15 flex items-center justify-center flex-shrink-0">
                <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse" />
              </div>
              <div>
                <p className="text-sm font-display font-semibold text-emerald-300">Currently available</p>
            ...
            <div className="space-y-3">
              {contactInfo.map(({ icon: Icon, label, value, href }) => (
                <div key={label} className="glass-card p-4 flex items-center gap-4 hover:border-primary-400/30 transition-all">
                  <div className="w-9 h-9 rounded-lg bg-primary-500/12 flex items-center justify-center flex-shrink-0">
            ...
            <div className="grid grid-cols-2 gap-3">
              {socialLinks.map(({ icon: Icon, label, href, desc }) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="glass-card p-4 flex items-center gap-3 hover:border-primary-400/30 hover:bg-primary-500/[0.05] transition-all group"
                >
                  <Icon size={18} className="text-slate-400 group-hover:text-primary-300 transition-colors" />
                  <div>
            ...
            <div className="glass-card p-6 sm:p-8">
              {sent ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <h3 className="text-base font-display font-semibold text-white mb-6">Send a message</h3>
